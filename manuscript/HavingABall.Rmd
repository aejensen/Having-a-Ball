...
We introduce the Excitement Trend Index (ETI) as an objective measure of spectator excitement in a given match.
...
Vi kan overveje, om vi ikke udelukkende burde fokusere på Eddy i dette manus og foreslå dette som et objektivt excitement index i stedet for at gå ind i en diskussion om TDI også. Jeg tænker også i forhold til at dette værk måske ikke behøver være så voluminøst.

Materials to reproduce this manuscript can be found at @HavingABall.
...
In Section XX we apply the ... of every match from the 2019-2020 NBA basketball season

# Methods {#sec:method}
Let $m$ index a given match between teams $a$ and $b$ and let $D_m(t_{mi}) = S_a(t_{mi}) - S_{b}(t_{mi})$ be the difference in scores at times $t_{m1} < t_{m2} < t_{mi} < \ldots < t_{m J_m}$ being the ordered event times when a score by either team $a$ or $b$ occurs  during match $m$.

We use the model
\begin{align}
\begin{split}
  (\bm{\beta}_m, \bm{\theta}_m, \sigma_{m}^2) \mid \bm{\Psi}_m, \mathbf{t}_m &\sim G(\bm{\Theta}_m \mid \bm{\Psi}_m, \mathbf{t}_m)\\
  f_m(t) \mid \bm{\Theta}_m &\sim \mathcal{GP}(\mu_{\bm{\beta}_m}(t), C_{\bm{\theta}_m}(t, t^\prime))\\
   D_m(t_{mi}) \mid f_m(t_{mi}), t_{mi}, \bm{\Theta}_m &\overset{iid}{\sim} N\left(f_m(t_{mi}), \sigma_{m}^2\right)
\end{split}
\end{align}
where $f_m$ models a latent trajectory of score differences. We also have that
\begin{align}
  \begin{bmatrix}f_m(s)\\ df_m(t)\\ d^2\!f_m(u)\end{bmatrix} \mid \bm{\Theta}_m &\sim \mathcal{GP}\left(\begin{bmatrix}\mu_{\bm{\beta}_m}(s)\\ d\mu_{\bm{\beta}_m}(t)\\ d^2\!\mu_{\bm{\beta}_m}(u)\end{bmatrix}, \begin{bmatrix}C_{\bm{\theta}_m}(s, s^\prime) & \partial_2 C_{\bm{\theta}_m}(s, t) & \partial_2^2 C_{\bm{\theta}_m}(s, u)\\ \partial_1 C_{\bm{\theta}_m}(t, s) & \partial_1 \partial_2 C_{\bm{\theta}_m}(t, t^\prime) & \partial_1 \partial_2^2 C_{\bm{\theta}_m}(t, u)\\ \partial_1^2 C_{\bm{\theta}_m}(u, s) & \partial_1^2\partial_2 C_{\bm{\theta}_m}(u, t) & \partial_1^2 \partial_2^2 C_{\bm{\theta}_m}(u, u^\prime)\end{bmatrix}\right)
\end{align}
where $d^k\!\mu_{\bm{\beta}}$ is the $k$'th derivative of $\mu_{\bm{\beta}}$ and $\partial_j^k$ denotes the $k$'th order partial derivative with respect to the $j$'th variable

We then define the Excitement Trend Index (ETI) as the number of zero-crossings of $df_m$ conditional on the observed score differences. Formally,
\begin{align*}
...
\begin{align*}
  \mu_{\bm{\beta}_m}(t) = \beta_{m}, \quad C_{\bm{\theta}_m}(t, t^\prime) = \alpha^2_m\exp\left(-\frac{(t-t^\prime)^2}{2\rho^2_m}\right)
\end{align*}
with $\alpha_m, \rho_m > 0$. 
...
\begin{figure}[htbp]
\missingfigure[figwidth=\textwidth]{1x3 plot of the last play-off. Observed scores with f, df and Teddy.}
\label{fig1}
\caption{Caption}
\end{figure}

\begin{figure}[htbp]
\missingfigure[figwidth=\textwidth]{1x2 plot of the distribution of mean posterior ETI (hist + density estimate) and mean posterior ETI as a function of calendar time.}
\label{fig2}
\caption{Caption}
\end{figure}

\begin{figure}[htbp]
\missingfigure[figwidth=\textwidth]{2x3 plot of game with minimum/median/maximum ETI where rows are observed scores + f and df}
\label{fig3}
\caption{Caption}
\end{figure}
...
We could define a weighted Excitement Trend Index, $\mathrm{ETI}^W_m$, so that zero crossings of the derivative of score differences are weighted higher towards the end of the game as in 
\begin{align*}
  \mathrm{ETI}_m^W = \int_{\mathcal{I}_m} d\mathrm{ETI}_m(t)w(t)\mathrm{d}t
\end{align*}
where $w$ is an increasing weight function.

Maybe changes in monotonicity of the score differences are not so important if one team is already far ahead of the other team. This would motivate a weighted ETI of the form
\begin{align*}
  \mathrm{ETI}_m^W = \int_{\mathcal{I}_m} d\mathrm{ETI}_m(t)w(t, |f_m(t)|)\mathrm{d}t
\end{align*}
where $w(\cdot, \cdot)$ is a decreasing function in its second variable.
