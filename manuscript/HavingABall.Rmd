...
\textbf{[??? The story we want to tell is ???]}

1) TDI can be used to assess trends within matches
2) ETI provides an objective way to rank different matches according to excitement
3) Summarizing ETI at the team level across matches in a season ranks the teams. We can estimate cuts in the ranking by collapsing the factor levels hence group teams according to average ETI
