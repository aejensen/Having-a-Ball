...
the 2019--2020 National Basketball Association (NBA) season and we show
...
We define the Trend Direction Index (TDI) of a particular match $m$ as the local posterior probability that $d_m$ is an increasing function for any time points $t \in \mathcal{I}_m$. Under our model this is equal to
\begin{align}
\begin{split}
...
\label{eq:ETIdef}
\end{align}
where $f_{d^\prime_m(t), d^{\prime\prime}_m(t)}(\cdot, \cdot \mid \mathcal{D}_m, \BTheta_m)$ denotes the posterior density function of $(d^\prime, d^{\prime\prime})$ at time $t$ according to Equation (\ref{eq:posterior}), and  $d\mathrm{ETI}_m$ is the instantaneous posterior probability of a zero-crossing of $d^\prime$ at any time point $t \in \mathcal{I}_m$. Under the model described in Equations (\ref{eq:hierarchicalmodel}) and (\ref{eq:dynamics}) it can be shown that the instantaneous posterior probability of a zero-crossing of $d^\prime$ is equal to
\begin{align*}
...
\end{gather*}
And therefore $\text{ETI}_m \mid \BTheta_m$ is given by integrating the previous expression of $d\mathrm{ETI}_m(t \mid \BTheta_m)$ over $t \in \mathcal{I}_m$. The derivation of the expression of $d\mathrm{ETI}_m$ can be found in the supplementary material to @ToT. We note that ETI is invariant with respect to the reference team in the definition of the score differences as it is defined as the expected number of both up- and down-crossings at zero of the posterior trend.
...
\end{align*}
where $T_{\text{df}}$ denotes a location-scale T distribution with $\mathrm{df}$ degrees of freedom, and $T_{\text{df}}^+$ is the distribution truncated to the positive real line. By the properties of the model in Equation (\ref{eq:hierarchicalmodel}) the marginal maximum log-likelihood function has the following closed-form expression
\begin{align*}
...
\end{align*}
and the marginal maximum likelihood estimate $\widehat{\BTheta_m^\text{ML}} = \argsup_{\BTheta} \log L(\BTheta \mid \mathcal{D}_m)$ is obtained by numerical optimization or as the roots to the score equations $\nabla_{\BTheta} \log L(\BTheta \mid \mathcal{D}_m) = 0$. 

We have implemented our model in the probabilistic programming language Stan [@carpenter2017stan] that uses a Hamiltonian MCMC (Markov Chain Monte Carlo) algorithm to sample from the posterior distributions $\textrm{TDI}_m(t \mid \widetilde{\BTheta}_m)$ and $d\mathrm{ETI}_m(t \mid \widetilde{\BTheta}_m)$ on any finite set of time points $\mathbf{t}_m^\ast \subset \mathcal{I}_m$. 
...
\textbf{[??? fix fitting stuff here. Er det ikke ok ???]}
For each match we ran four independent chains for 50,000 iterations each with half of the iterations used for warm-up. Using 241 equidistant grid points in $[0; 48]$. The posterior distribution of $\text{ETI}_m \mid \widetilde{\BTheta}_m$ was calculated by numerical integration of $d\mathrm{ETI}_m(t \mid \widetilde{\BTheta}_m)$ using the trapezoidal rule on the grid of time points.
...
\caption{Results from fitting the latent Gaussian Process to the final match between LA Lakers and Miami Heat in the 2019--2020 NBA season. Larger values in running score differences and the Trend Direction Index reflect the situation where LA Lakers are doing better. The top left panel shows the posterior distribution of the the latent process with the posterior means in bold. The gray areas show point-wise credibility intervals for the posterior distribution. The top right panel provides similar information for the derivative of the latent process, i.e., the posterior trend. The bottom left panel shows the Trend Direction Index and can be used to read off probability statements about the trends in the running score difference. The bottom right panel shows the local Excitement Trend Index and quantifies the instantaneous probability of a change in monotonicity of the score differences.}
\label{fig1b}
\end{figure}
...
When matches are ranked from lowest to highest ETI, we can extract the individual analyses for matches representing the full span of the ETI range. Figure \ref{fig3} shows the analysis results of our proposed method for the matches with minimum, 1st, 2nd, 3rd quantile, and maximum ETI. It is quite clear from the observed running score differences, posterior trends and the TDIs that these five matches represent substantially different game experiences. The third row of Figure  \ref{fig3} for the LA Clippers vs Charlotte Hornets match indicates that while the Hornets did lead throughout most of first quarter, the Clippers reversed the game towards the end of first quarter and kept the lead through most of 2nd quarter. After that the Hornets started to keep the lead and the Clippers never managed to make a proper comeback and the game trend was rather flat in the last two quarters of the game since the two teams more or less kept the pace with each other except for the effort shown midway in quarter four. In contrast, the match between New Orleans Pelicans vs Utah Jazz (5th row in Figure \ref{fig3}) showed trends that varied direction frequently and where the TDI showed alternating periods of scoring bursts making it a very exciting and unpredictable game.
...
The problem is mathematically equivalent to looking at the relationship between the median ETIs as the outcome in a linear regression model where the explanatory categorical variable ranges over the set of all partitions of the 30 teams, and as the objective we seek the smallest number of partitions that best explains the observed ETIs by comparing all possible splits of the ranked teams for a given number of partitions. This will then define subgroups of teams.

As an optimization criterion we used root mean squared error of predicting based on leave-one-out cross-validation, denoted $\textrm{RMSEP}_\text{LOO-CV}$.
Our sequential optimization procedure showed that the leave-one-out cross-validated root mean squared error of prediction stabilized at four subgroups: $\textrm{RMSEP}_\text{LOO-CV}^{C=2} = `r load("../results/teams_grouped_by_ETI.RData"); signif(part_2$optim$bestval, 4)`$, $\textrm{RMSEP}_\text{LOO-CV}^{C=3} = `r load("../results/teams_grouped_by_ETI.RData"); signif(part_3$optim$bestval, 4)`$, $\textrm{RMSEP}_\text{LOO-CV}^{C=4} = `r load("../results/teams_grouped_by_ETI.RData"); signif(part_4$optim$bestval, 4)`$, and subsequently for $C = 5,\ldots,8$ it remained at the same value. The labels of these groups are shown in the rightmost column in Table  \ref{tab:teamsummary}. The result is thus an identification of three estimated change-points in the ranking of the teams according to the average median posterior ETI. The most noticeable result is that Charlotte Hornets constitute a singleton since that team has substantial lower ETI than the team with the second lowest ETI. To have a higher probability of seeing an exciting game it would be wise to avoid the Hornets. 

Figure 5 shows the estimated linear association between the seasonal standard deviations of the median posterior ETIs and the seasonal average of the median posterior ETIs for each team across their matches. The association follows a straight line through the origin \textbf{[??? Origin? (Intercept) = 0?]} fairly well, suggesting that the coefficient of variation of seasonal excitement is fairly constant across the teams. This also suggests that teams with large seasonal average ETI are also more likely to be part of really spectacular games with high ETIs. Teams consistently contributing with highly exciting matches should therefore be represented in the bottom right part of the plot. Assessing which teams that where generally most exiting during the season is a bias-variance trade-off decision rule. While the New Orleans Pelicans had the highest seasonal average ETI, the Washington Wizards had the second highest seasonal average but also a notably lower seasonal standard deviation. This means that although the latter team was less exciting on average during the season, their level of excitement was more stable throughout the season. On the other hand, the San Antonio Spurs had the third largest seasonal average ETI but also a much larger seasonal standard deviation making them less excitement reliable throughout the season.


# Discussion {#sec:discussion}

We have introduced the Trend Direction Index as a measure to estimate and evaluate the trends in running score difference in sports. The Trend Direction Index is based on a latent Gaussian process and enables us to make Bayesian in-game and post-game evaluations about the underlying trends in scoring patterns and to attach easily interpretable probability statements to the results. 
In addition we have presented the Excitement Trend Index (ETI) which is the expected number of monotonicity changes in the underlying trend and we showed how it can be used to gauge how exciting a match will be: if one team is consistently outperforming the other then the match quickly becomes one-sided and less exciting. Both indices have intuitive interpretations that are easily conveyed to non-statisticians, coaches, players, and commentators. 
...
In conclusion, we have provided analytical framework for analyzing the trend in the running score difference in sports matches. The latent Gaussian process requires very few assumptions which makes the modeling approach very flexible and applicable to a multitude of sports.
...
```{r linplot2, echo=FALSE, message=FALSE, fig.cap="Ohoy \\textbf{[??? Can we make the points different (like pch) so that the plot can be read in black/white and also the average line in black?. Ticks at the x-axis could also be improved]}"}
