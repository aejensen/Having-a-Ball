...
Summarizing the posterior median ETIs at the team level lead to comparable values across all 30 teams. The New Orleans Pelicans had the highest average posterior median ETI during the season with a value of 11.67 (SD = 4.91, IQR = $[3.09; 23.57]$), while the Charlotte Hornets had the lowest average posterior median ETI during the season with a value of 9.29 (SD = 3.74, IQR = $[1.96; 15.98]$). This implies that each team played games during the season that where comparable on average in terms their excitement, and the major source of variation in excitement is governed by the individual matches. Table \ref{tab:teamsummary} shows the summary statistics for all 30 teams.

\textbf{[??? Insert stuff about collapsing a categorical covariate here ???]}
...
tab$Class <- tab$class_num
tab$Class[tab$class_num == "2"] <- "A"
tab$Class[tab$class_num == "0"] <- "B"
tab$Class[tab$class_num == "3"] <- "C"
tab$Class[tab$class_num == "1"] <- "D"
tab$class_num <- NULL
...
kable(tab, 
      ...
      linesep = c("", "", "", ""), 
      align = c("r", "r|", "r", "r", "r|", "r"),
			caption = "Team specific median posterior Excitement Trend Indices summarized across all their matches in the 2019--2020 NBA season ordered by the season average. Additionally, standard deviations and 2.5\\%, 50\\%, and 97.5\\% percentiles are shown. The class column shows the group to which each team was assigned by the .... algorithm.\\label{tab:teamsummary}")
```
