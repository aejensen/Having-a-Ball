...
\label{eq:ETIdef}
\end{align}
where $f_{d^\prime_m(t), d^{\prime\prime}_m(t)}(\cdot, \cdot \mid \mathcal{D}_m, \BTheta_m)$ denotes the posterior density function of $(d^\prime, d^{\prime\prime})$ at time $t$ and  $d\mathrm{ETI}_m$ is the instantaneous posterior probability of a zero-crossing of $d^\prime$ at any time point $t \in \mathcal{I}_m$. Under the model described in Equations (\ref{eq:hierarchicalmodel}) and (\ref{eq:dynamics}) it can be shown that $d\mathrm{ETI}$ is equal to
\begin{align*}
...
\end{gather*}
A derivation of these expressions can be found in the supplementary material to @ToT. We note that ETI is invariant with respect to the reference team in the definition of the score differences as it is defined as the expected number of both up- and down-crossings at zero of the posterior trend.
