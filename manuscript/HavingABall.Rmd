...
\begin{split}
  \textrm{TDI}_m(t \mid \BTheta_m) &= P\left(d_m^\prime(t) > 0 \mid \mathcal{D}_m, \BTheta_m\right)\\
     &= \frac{1}{2} + \frac{1}{2}\Erf\left(\frac{\mu_{d_m^\prime}(t \mid \BTheta_m)}{2^{1/2}\Sigma_{d_m^\prime}(t, t \mid \BTheta_m)^{1/2}}\right)
\end{split}
\label{eq:TDIdef}
\end{align}
